...
export default function Index() {
	...
	const isLoading = todoFetcher.state === "idle" && todoFetcher.data;
	...
	useEffect(() => {
		if (isLoading && todoFetcher.data.id !== tasks[tasks.length - 1]?.id) {
			setTasks([...tasks, todoFetcher.data]);
		}
	}, [setTasks, tasks, isLoading, todoFetcher.data]);
				...
				{tasks.length > 0 ? (
					<>
						...
						<div className="flex flex-col gap-y-2">
							...
							{isSubmitting && (
								<div className="zinc-800 flex items-center gap-x-2 rounded-md border border-zinc-600 bg-zinc-900 bg-gradient-to-r p-2 px-4 shadow">
									<div role="status">
										<svg
											aria-hidden="true"
